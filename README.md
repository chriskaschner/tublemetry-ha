...
Home Assistant package YAML files for the [Tublemetry](https://github.com/chriskaschner/tublemetry) hot tub automation system. The Git Pull add-on syncs this repo to `/config/packages/` on your HA instance.

## Files

| File | Description |
|------|-------------|
| `tou_automation.yaml` | Time-of-Use setpoint schedule (Rg-2A rate windows) |
| `thermal_runaway.yaml` | Graduated thermal runaway protection (warning/moderate/severe) |
| `thermal_runaway_clear.yaml` | Auto-clears runaway flag when temperature returns to setpoint |
| `stale_data.yaml` | ESP32 offline detection -- disables TOU on stale sensor data |
| `drift_detection.yaml` | Detects setpoint drift (physical button press or failed injection) |
| `heater_power.yaml` | Enphase-derived heater power validation binary sensor (stubbed) |
| `sensors.yaml` | SQL sensors: heating rate and cooling rate from recorder history |
| `templates.yaml` | Template sensors: outdoor temp, preheat minutes, preheat ETA |
| `thermal_model.yaml` | Full thermal model: SQL rates, template estimates, input_number snapshots, daily refresh automation |
| `helpers.yaml` | Input boolean: `thermal_runaway_active` flag |
...
| Deployment directory | `/config/packages` |
